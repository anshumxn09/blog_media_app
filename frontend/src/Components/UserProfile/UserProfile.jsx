...
const UserProfile = ({imageUrl, name, id, search=false, isVertical=false}) => {
  return (
    <Link style={{background : "white", width : search && "100%", borderRadius : search && "20px", padding : search & '10px 5px', flexDirection : isVertical ? "column" : "row"}} className="profile-row" to={`/user/${id}`}>
